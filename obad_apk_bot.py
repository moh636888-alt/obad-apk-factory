...
def get_failed_job_log(run_id):
    """يجلب سجل الخطأ الكامل مباشرة من GitHub API، بلا حاجة لفتح المتصفح."""
    jobs_url = f"{GITHUB_API}/repos/{GITHUB_OWNER}/{GITHUB_REPO}/actions/runs/{run_id}/jobs"
    resp = requests.get(jobs_url, headers=HEADERS)
    jobs = resp.json().get("jobs", [])
    if not jobs:
        return None

    # نبحث عن أول Job فاشل تحديدًا (وليس أول واحد فقط)
    failed_job = next((j for j in jobs if j.get("conclusion") == "failure"), jobs[0])
    job_id = failed_job["id"]

    logs_url = f"{GITHUB_API}/repos/{GITHUB_OWNER}/{GITHUB_REPO}/actions/jobs/{job_id}/logs"
...
def extract_error_excerpt(full_log, max_chars=1200):
    """يستخرج أهم أسطر الخطأ من سجل طويل، بدل إرساله كاملًا في نص الرسالة."""
    lines = full_log.splitlines()
    error_lines = [
        l for l in lines
        if any(k in l for k in ("Error", "error:", "FAILED", "Exception", "##[error]"))
    ]
    excerpt = "\n".join(error_lines[-25:]) if error_lines else "\n".join(lines[-25:])
...
async def handle_zip(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not run_id:
        # حتى هنا، نحاول جلب آخر Run موجود لعرض تفاصيله إن وُجد
        await update.message.reply_text(
            "⚠️ تعذر العثور على عملية بناء بعد التشغيل. قد تكون بدأت متأخرة قليلاً — "
            "افحص لاحقًا في تليجرام برسالة جديدة، أو تحقق من صلاحيات GITHUB_TOKEN."
        )
        shutil.rmtree(work_dir, ignore_errors=True)
        return
    ...
    if conclusion != "success":
        ...
        full_log = get_failed_job_log(run_id)
        ...
        if full_log:
            ...
            # نرسل أيضًا السجل الكامل كملف نصي احتياطًا
            log_path = os.path.join(work_dir, "build_error_log.txt")
            with open(log_path, "w", encoding="utf-8") as f:
                f.write(full_log)
            await update.message.reply_document(document=open(log_path, "rb"))
        else:
            await update.message.reply_text(
                f"❌ فشل البناء (النتيجة: {conclusion}).\nتعذر جلب السجل تلقائيًا، راجع الرابط:\n{run_url}"
            )
        shutil.rmtree(work_dir, ignore_errors=True)
        return
    ...
    if apk_path and os.path.exists(apk_path):
        ...
        # نعرض للمستخدم فعليًا أي ملفات وُجدت، بدل رسالة غامضة
        found_files = []
        for root, _, files in os.walk(work_dir):
            for name in files:
                found_files.append(os.path.relpath(os.path.join(root, name), work_dir))
        listing = "\n".join(found_files[:30]) if found_files else "لا توجد أي ملفات ناتجة."
        await update.message.reply_text(
            f"⚠️ نجح البناء لكن تعذر العثور على ملف APK بالاسم المتوقع.\n"
            f"الملفات الموجودة فعليًا:\n{listing}"
        )
